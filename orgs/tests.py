...
class EmojiPickerComesFromADataFileTests(TestCase):
    """"Add all emoji, let me be able to sort the way I see in WhatsApp."

    All of them is not a thing to type into a template. The list is generated by
    scripts/build_emoji.py from the file the Unicode Consortium publishes for
    keyboards (UTS #51 emoji-test.txt), so the categories are the ones every
    phone already uses rather than somebody's guess, and every character arrives
    with its official name to search by.
    """
    ...
    def setUpTestData(cls):
        ...
        cls.path = settings.BASE_DIR / "static" / "data" / "emoji.json"
    ...
    def test_the_file_is_there_and_is_the_whole_set(self):
        self.assertTrue(self.path.exists(), "run scripts/build_emoji.py")
        data = self._data()
        total = sum(len(g["emoji"]) for g in data["groups"])
        # 1,870 at Unicode 15.0. Asserted as a floor rather than an equality so
        # regenerating against a newer release is not a test failure — what this
        # guards is somebody quietly replacing it with a hand-picked shortlist.
        self.assertGreater(total, 1500, f"only {total} emoji — is this the full file?")

    def test_the_categories_are_the_ones_keyboards_use(self):
        keys = [g["key"] for g in self._data()["groups"]]
        self.assertEqual(
            keys,
            ["smileys", "nature", "food", "activity", "travel", "objects", "symbols", "flags"],
        )
        for g in self._data()["groups"]:
            self.assertTrue(g["label"] and g["tab"], f"{g['key']} has no label or tab")

    def test_every_emoji_carries_its_name_to_search_by(self):
        for g in self._data()["groups"]:
            for char, name in g["emoji"][:40]:
                self.assertTrue(char and name, f"empty row in {g['key']}")
                self.assertEqual(name, name.lower(), "names are matched lowercased")

    def test_skin_tone_variants_are_left_out(self):
        """Five copies of every hand is a tone picker's job, not a reason to
        make the grid five times longer."""
        tones = {"\U0001F3FB", "\U0001F3FC", "\U0001F3FD", "\U0001F3FE", "\U0001F3FF"}
        for g in self._data()["groups"]:
            for char, name in g["emoji"]:
                self.assertFalse(
                    tones.intersection(char), f"{name} is a skin-tone variant",
                )

    def test_nothing_appears_twice(self):
        seen = {}
        for g in self._data()["groups"]:
            for char, name in g["emoji"]:
                self.assertNotIn(char, seen, f"{name} is also in {seen.get(char)}")
                seen[char] = g["key"]

    def test_the_composer_points_at_the_file_and_does_not_inline_the_list(self):
        """The shell is markup; the list is not. It used to be 1,300 characters
        re-sent inside every render of the chat pane."""
        from django.template.loader import render_to_string
        from unittest.mock import Mock

        html = render_to_string("orgs/partials/_room_composer.html", {
            "thread": Mock(id=1, kind="org", group=None, **{"status": "open"}),
            "thread_org": Mock(id=1, name="Test Co"),
            "other": None,
        })
        self.assertIn("data-emoji-src", html)
        self.assertIn("emoji.json", html)
