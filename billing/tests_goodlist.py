...
from catalog.models import Charity, GoodListConfig, OrganisationType, Season, State, SubCategory
...
class GoodListTests(TestCase):
    ...
    def setUp(self):
        ...
        self.education = OrganisationType.objects.get(slug="education")
    ...
    def make_group(self, name, *, amount, charity=None, state=None, sub_categories=(),
                   organisation_type=None, settled=True, public=False):
    ...
    def test_type_filter_separates_surfaces(self):
        self.make_group("Club A", amount="100", public=True, organisation_type=self.community)
        self.make_group("Club B", amount="100", public=True, organisation_type=self.community)
        self.make_group("Corp A", amount="500", public=True, organisation_type=self.business)
        self.make_group("Corp B", amount="500", public=True, organisation_type=self.business)
        community = [r["name"] for r in goodlist.by_group(organisation_type_slug="community")]
    ...
    def test_sub_category_filter_and_education_dual_surface(self):
        ...
        primary = SubCategory.objects.get(organisation_type=self.education, slug="primary-school")
        secondary = SubCategory.objects.get(organisation_type=self.education, slug="secondary-school")
        finance = SubCategory.objects.get(organisation_type=self.business, slug="finance")
        self.make_group("Both Ways College", amount="400", public=True,
                        organisation_type=self.education, sub_categories=[primary, secondary])
        self.make_group("Finance Co", amount="200", public=True,
                        organisation_type=self.business, sub_categories=[finance])
    ...
    def test_informal_label_shows_as_category(self):
        informal = OrganisationType.objects.get(slug="informal")
        org = self.make_group("The Crew", amount="100", public=True, organisation_type=informal)
