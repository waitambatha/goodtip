...
Recap text is generated by an LLM call, not templated fill-in-the-blank
sentences. Templating reads as robotic within two or three rounds; a model
call, constrained by a tight prompt and real numbers, keeps it readable at
round fifteen the same as round one.

- **Provider:** Anthropic API is the reference implementation. The approach
  is provider-agnostic, structured JSON in, constrained plain text out,
  batched generation, so it runs the same way through OpenAI's API or any
  comparable provider. This is Ambrose's build choice on cost and
  reliability, not a requirement of this spec.
- **Input to the model:** structured JSON only, built server-side from the
  data in Sections 4 and 5. Never send free text or unvetted user content
  into the prompt.
- **Output from the model:** plain text, 2 to 4 sentences, no headline, no
  emoji, no hashtags.
- **Voice constraint:** the prompt must enforce GoodTip's brand voice rules
  directly (see Sections 8 and 9), not leave tone to the model's default.
- **Cost and reliability:** batch all of a round's group recaps in one
  generation pass after lock, not real-time on page load. A failed generation
  should fall back to a simple factual line (see Section 10), never a blank
  card or a visible error.
...
| Model call fails | Fallback to one factual line built without the LLM: top scorer's name and round points only. |
